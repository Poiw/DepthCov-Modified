...
class NonstationaryGpModule(pl.LightningModule):
  ...
  def est_sparse_depth(self, gaussian_covs, sparse_train_coords_norm, sparse_train_log_depth, mean_depth, sparse_test_coords_norm, level=-1):
    
    if level == -1:
      level = len(gaussian_covs)-1
    
    sparse_var_level = self.get_var(level)
    sparse_vars = sparse_var_level*torch.ones_like(sparse_train_log_depth)
    sparse_vars = sparse_vars.squeeze(-1)

    E_train = gk.interpolate_kernel_params(gaussian_covs[level], sparse_train_coords_norm)
    E_test = gk.interpolate_kernel_params(gaussian_covs[level], sparse_test_coords_norm)

    K_train_train = self.cov_modules[level](sparse_train_coords_norm, E_train)
    K_train_test = self.cross_cov_modules[level](sparse_train_coords_norm, E_train, sparse_test_coords_norm, E_test)
    K_test_test_diag = self.diagonal_cov_modules[level](sparse_test_coords_norm, E_test)

    L, alpha, _, info = self.gp_train_module(K_train_train, sparse_train_log_depth, mean_depth, sparse_vars)
    if info.any():
      print("Cholesky failed")
    pred_depth, pred_var = self.gp_test_module(L, alpha, K_train_test, K_test_test_diag, mean_depth)
    pred_depth = torch.permute(pred_depth, (0,2,1))
